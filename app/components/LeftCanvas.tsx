...
const LeftCanvas = ({setNumber_of_bars, number_of_bars, randomButton, bubbleSort, time, setTime, selectionSort, insertionSort,mergeSort,sorting, timetosort}: props) => {
  return (
    <>
    <div className="bg-gray-700 p-2 w-1/4 h-screen">
        ...
        <div className="p-5 rounded-md bg-gray-800 mb-2">
            ...
            <input disabled={sorting} id="default-range" type="range"
             value={number_of_bars} max={100} min={2} onChange={(e) => setNumber_of_bars(Number(e.target.value))} 
             className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"/>
            <button disabled={sorting} onClick={() =>  randomButton()} className='text-center p-3 disabled:bg-gray-600 my-2 rounded-md text-2xl bg-black w-full'>
              Randomize</button>
        </div>
        <div className="p-5 rounded-md bg-gray-800 mb-2">
        <label htmlFor="default-range" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Set Sorting time</label>{time}ms
            ...
            <button disabled={sorting} onClick={() => insertionSort()} className="text-center p-3 my-2 disabled:bg-gray-600 rounded-md text-2xl bg-black w-full">Insertion Sort </button>
            <button disabled={sorting} onClick={() => mergeSort()} className="text-center p-3 my-2 disabled:bg-gray-600 rounded-md text-2xl bg-black w-full">Merge Sort </button>
        </div>
